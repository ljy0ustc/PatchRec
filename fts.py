...
from fts_trainer import FTSTrainer
...
def parser():
    ...
    parser.add_argument('--output_dir', type=str, default="output/fts")
    parser.add_argument('--logging_dir', type=str, default="logs/fts")
...
def train(args):
    ...
    args.output_dir = op.join(args.output_dir, args.dataset, f"sft-epoch{args.num_train_epochs}-sl{args.truncate_seq}-gs{args.group_size}-bs{args.batch_size}-gas{args.gradient_accumulation_steps}-lr{args.learning_rate}")
    args.logging_dir = op.join(args.logging_dir, args.dataset, f"sft-epoch{args.num_train_epochs}-sl{args.truncate_seq}-gs{args.group_size}-bs{args.batch_size}-gas{args.gradient_accumulation_steps}-lr{args.learning_rate}")
